...
class Bot(BaseBot):
    async def on_start(self, session_metadata: SessionMetadata) -> None:
        print("working")
        await self.highrise.walk_to(Position(7.5 , 13.75 , 4.0 , "FrontLeft"))
    ...
    async def on_chat(self, user: User, message: str) -> None:
        ...
        if message.lower().startswith("!tipall ") and user.username == "Buck3ts" or user.username == "coolbuns":
              parts = message.split(" ")
              if len(parts) != 2:
        ...
        if message.lower().startswith("!tipme ") and user.username == "Buck3ts" or user.username == "coolbuns":
                try:
        ...
        if message.startswith("!1"):
            await self.highrise.teleport(user.id, Position(11.5 , 0.75 ,13.5 ))

        if message.startswith("!2"):
            await self.highrise.teleport(user.id, Position(11.0 , 7.75 , 4.0))

        if message.startswith("!3"):
            await self.highrise.teleport(user.id, Position(12.5 , 13.75 , 13.5))
        ...
        if        message.startswith("/tele") or              message.startswith("/tp") or              message.startswith("/fly") or     message.startswith("!tele") or      message.startswith("!tp") or     message.startswith("!fly"):
          if user.username == "Buck3ts" or user.username == "coolbuns":            await self.teleporter(message)
        ...
        if              message.startswith("Fairyfloat All") or                              message.startswith("/emote all fairyfloat") or       message.startswith("!emote all fairyfloat"):
          if user.username == "Buck3ts" or user.username == "coolbuns":
        ...
        if message.startswith("!kick"):
          ...
          if len(parts) != 2:
              await self.highrise.chat("wrong ban format. ")
              return
