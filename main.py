...
class Bot(BaseBot):
    ...
    async def on_chat(self, user: User, message: str) -> None:
        ...
        if message.lower().startswith("!tipall ") and user.username == "Buck3ts":
              parts = message.split(" ")
              if len(parts) != 2:
        ...
        if message.lower().startswith("!tipme ") and user.username== "Buck3ts":
                try:
        ...
        if        message.startswith("/tele") or              message.startswith("/tp") or              message.startswith("/fly") or     message.startswith("!tele") or      message.startswith("!tp") or     message.startswith("!fly"):
          if user.username == "Buck3ts":            await self.teleporter(message)
